...
tf.flags.DEFINE_string("logs_dir", "logs20180526_soft/", "path to logs directory")
...
tf.flags.DEFINE_string("data_dir", "image_save20180526_soft", "path to dataset")
#the learning rate
...
def main(argv=None):
    ...
    for itr in xrange(MAX_ITERATION):
        ...
        feed_dict = {image: train_images, soft_annotation: train_annotations/100, keep_probability: 0.85}

        sess.run(train_op, feed_dict=feed_dict)
        logs_file = open(path_, 'a')  
        train_annotations_=train_annotations/100
        if itr % 10 == 0:
            '''
            for ii in range(224):
                for jj in range(224):
                    if train_annotations_[0][ii][jj][1] > 0:
                        print('anno',ii,', ', jj,': ', train_annotations_[0][ii][jj])
            '''
            #train_softmax = sess.run([soft_logits], feed_dict=feed_dict)
        ...
        if itr % 500 == 0:
            ...
            train_loss,train_pred_anno = sess.run([soft_loss,pred_annotation], feed_dict={image:train_random_images,
                                                                                        soft_annotation:train_random_annotations/100,
